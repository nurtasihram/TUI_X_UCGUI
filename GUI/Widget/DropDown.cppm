...
void DROPDOWN_Expand(DROPDOWN_Handle hObj) {
	...
	if (pLst == 0) {
		...
		if (pLst) {
			if (pObj->Flags & DROPDOWN_SF_AUTOSCROLLBAR) {
				pLst->SetScrollbarWidth(pObj->ScrollbarWidth);
	...
	if (pLst) {
		...
		for (i = 0; i < NumItems; i++)
			pLst->AddString(_GetpItem(pObj, i));
		for (i = 0; i < GUI_COUNTOF(pObj->Props.aBkColor); i++)
			pLst->SetBkColor(i, pObj->Props.aBkColor[i]);
		for (i = 0; i < GUI_COUNTOF(pObj->Props.aTextColor); i++)
			pLst->SetTextColor(i, pObj->Props.aTextColor[i]);
		pLst->SetItemSpacing(pObj->ItemSpacing);
		pLst->SetFont(pObj->Props.pFont);
		pLst->SetSel(pObj->Sel);
...
void DROPDOWN_SetBkColor(DROPDOWN_Handle hObj, unsigned int Index, RGBC color) {
	...
	if (Index < GUI_COUNTOF(pObj->Props.aBkColor)) {
		...
		WM_Invalidate(hObj);
		if (pObj->pListWin) {
			pObj->pListWin->SetBkColor(Index, color);
		}
	}
}
void DROPDOWN_SetTextColor(DROPDOWN_Handle hObj, unsigned int Index, RGBC color) {
	...
	if (Index < GUI_COUNTOF(pObj->Props.aBkColor)) {
		...
		WM_Invalidate(hObj);
		if (pObj->pListWin) {
			pObj->pListWin->SetTextColor(Index, color);
		}
	}
}
void DROPDOWN_SetSel(DROPDOWN_Handle hObj, int Sel) {
...
void DROPDOWN_SetScrollbarWidth(DROPDOWN_Handle hObj, unsigned Width) {
	...
	if (Width != (unsigned)pObj->ScrollbarWidth) {
		...
		if (pObj->pListWin) {
			pObj->pListWin->SetScrollbarWidth(Width);
		}
	}
}
...
void DROPDOWN_DeleteItem(DROPDOWN_Handle hObj, unsigned int Index) {
	...
	if (Index < NumItems) {
		...
		WM_Invalidate(hObj);
		if (pObj->pListWin) {
			pObj->pListWin->DeleteItem(Index);
		}
	}
}
void DROPDOWN_InsertString(DROPDOWN_Handle hObj, const char *s, unsigned int Index) {
	if (s) {
		...
		if (Index < NumItems) {
			...
			WM_Invalidate(hObj);
			if (pObj->pListWin) {
				pObj->pListWin->InsertString(s, Index);
			}
		}
		else {
			DROPDOWN_AddString(hObj, s);
			if (pObj->pListWin) {
				pObj->pListWin->AddString(s);
			}
		}
	}
}
void DROPDOWN_SetItemSpacing(DROPDOWN_Handle hObj, unsigned Value) {
	...
	if (pObj->pListWin) {
		pObj->pListWin->SetItemSpacing(Value);
	}
}
uint16_t DROPDOWN_GetItemSpacing(DROPDOWN_Handle hObj) {
	auto pObj = (DROPDOWN_Obj *)hObj;
...
void DROPDOWN_SetAutoScroll(DROPDOWN_Handle hObj, int OnOff) {
	...
	if (pObj->Flags != Flags) {
		pObj->Flags = Flags;
		if (pObj->pListWin) {
			pObj->pListWin->SetAutoScrollV((Flags & DROPDOWN_SF_AUTOSCROLLBAR) ? 1 : 0);
		}
	}
}
void DROPDOWN_SetTextAlign(DROPDOWN_Handle hObj, int Align) {
